...
# 增加一门课程
def _add_new_course(clsname, class_time, row_number, teacher, duration, week, location, md5_value, xh):
    if settings.DEBUG:
...
def process():
    global table1_count_add, table1_count_pass, table2_count_add, table2_count_pass
                ...
                for row_number in range(1, 7):
                    ...
                    for i in soup.select(query_selector):  # i 为 a 元素
                        ...
                        if not class_fetch_result:
                            _add_new_course(str(class_info['clsname']),
                                            class_time,
                                            row_number,
                                            str(class_info['teacher']),
                                            str(class_info['duration']),
